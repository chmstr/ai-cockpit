...
export const RULES_COLLECTION_ID = "rules"; // Replace with your collection ID
...
export const ens = reactive({
  ...
  async init() {
    ...
    allmeasures.deleteByCollectionId(RULES_COLLECTION_ID);
    allmeasures.update(this.current);
    console.log("init RULE done");
  ...
  async remove(id) {
    await databases.deleteDocument(KICD_DATABASE_ID, RULES_COLLECTION_ID, id);
    this.current = this.current.filter((idea) => idea.$id !== id);
    await this.init(); // Refetch ideas to ensure we have 10 items
  },
  async update(idea) {
    console.log(idea);
    await databases.updateDocument(KICD_DATABASE_ID, RULES_COLLECTION_ID, idea.$id, idea);
    await this.init(); // Refetch ideas to ensure we have 10 items
  },
});
